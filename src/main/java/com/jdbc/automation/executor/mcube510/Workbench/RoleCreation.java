        ...
        try {
            ...
            if (flag == 1) {
                ...
                if (SeleniumUtils.isRoleCreated(driver, wait, roleName)) {
                    ...
                    try {
//                         driver.findElement(By.cssSelector("div[class='col-sm-offset-4 col-sm-5 workbench-title']"));
                        List<WebElement> userList = driver.findElements(By.xpath("//ul[@class='user-list-height']/li"));
                        int n = userList.size();
                        webEl = userList.get(n-1);
                        System.out.println("Inside try for scrolling..");
                    } catch (Exception e) {
                        webEl = driver.findElement(By.xpath("//div[@class='col-sm-offset-4 col-sm-5 workbench-title']"));
                    }
                ...
                System.out.println("Actuall Outcome for Role : "+actualOutcome);
                //--------------------------------------
            }

            List<WebElement> dms = driver.findElements(By.cssSelector("div[class='menupad-left']"));
            System.out.println("DMSS size : "+dms.size());
            dms.get(1).click();
            System.out.println("Management is clickedd for User to be created");
            System.out.println("Management is clicked");
            System.out.println("------------------------------------------------------------------------------------------");
        } catch (Exception e) {
            System.out.println("Inside CCATH");
            screenShot.add(SeleniumUtils.takeScreenShot(driver));
            actualOutcome = false;
            log.error("",e);
        }
