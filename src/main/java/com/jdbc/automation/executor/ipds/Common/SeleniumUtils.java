package com.jdbc.automation.executor.ipds.Common;
...
public class SeleniumUtils {
    ...
    public static void changeDate(WebDriver driver, WebDriverWait wait, List<File> screenShot,String module) throws Exception {
        ...
        try {
            Thread.sleep(4000);
            //driver.findElement(By.xpath("//*[@id=\"KGTPTimePickerBtnID\"]/pretty-duration")).click();
            List<WebElement> downArrow = driver.findElement(By.cssSelector("button[class='button']")).findElements(By.cssSelector("i[class='fa fa-angle-down']"));
            System.out.println("Down Arrow Size : "+downArrow.size());
            if(!module.equalsIgnoreCase("Dashboard")) {
                downArrow.get(0).click();
                Thread.sleep(1000);
            }
            else {
                System.out.println("About to click down arrow");
                driver.findElement(By.xpath("//div[@class='col-sm-3 p-0']/div[@class='navbar-timepicker-wrapper']/button/i")).click();
                System.out.println("Down Arrow clicked for Dashboard");
                Thread.sleep(1000);
            }
            List<WebElement> monthList = driver.findElements(By.cssSelector("div[ng-switch='datepickerMode']"));
            monthList.get(0).findElement(By.cssSelector("button[class='btn btn-default btn-sm']")).click();
            Thread.sleep(1000);
            monthList.get(0).findElement(By.cssSelector("button[class='btn btn-default btn-sm']")).click();
            Thread.sleep(1000);
            //Left arrow
            monthList.get(0).findElement(By.cssSelector("i[class='fa fa-angle-left']")).click();
            Thread.sleep(1000);
            List<WebElement> yearList = driver.findElement(By.cssSelector("table[ng-switch-when='year']")).findElements(By.tagName("tbody"));
            List<WebElement> yearSubtr = yearList.get(0).findElements(By.tagName("tr"));
            List<WebElement> yearSubtd = yearSubtr.get(0).findElements(By.tagName("td"));
            yearSubtd.get(2).findElement(By.tagName("button")).click();

            Thread.sleep(2000);
            List<WebElement> monthhList = driver.findElement(By.cssSelector("table[ng-switch-when='month']")).findElements(By.tagName("tbody"));
            List<WebElement> monthSubtr = monthhList.get(0).findElements(By.tagName("tr"));
            List<WebElement> monthSubtd = monthSubtr.get(0).findElements(By.tagName("td"));
            monthSubtd.get(2).findElement(By.tagName("button")).click();
            System.out.println("March be cliucked");
            Thread.sleep(2000);

            List<WebElement> dayyList = driver.findElement(By.cssSelector("table[ng-switch-when='day']")).findElements(By.tagName("tbody"));
            List<WebElement> dayySubtr = dayyList.get(0).findElements(By.tagName("tr"));
            List<WebElement> dayySubtd = dayySubtr.get(1).findElements(By.tagName("td"));
            dayySubtd.get(2).findElement(By.tagName("button")).click();

            driver.findElement(By.cssSelector("button[class='button kbn-timepicker-apply']")).click();
            Thread.sleep(3000);
            ...
            screenShot.add(SeleniumUtils.takeScreenShot(driver));
            actualOutcome = false;
            System.out.println("Change Date failed");
            //throw new RuntimeException(e);
        }
    }
...
//        OutputStream out = new FileOutputStream((SrcFile));
    ...
    public static boolean selectIndices(WebDriver driver, WebDriverWait wait, String index) {
        ...
        try {
            ...
            List<WebElement> list = driver.findElement(By.cssSelector("div[class='inputpart']")).findElements(By.cssSelector("li[ng-class=\"dataset.class\"]"));
            int size = list.size();
            System.out.println("SIZE of Dataset List: " + size);
    ...
    public static boolean query(WebDriver driver, WebDriverWait wait, String index) {
        ...
        try {
            wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("mentio-menu[mentio-search='bridgeSearch(term)']")));

            List<WebElement> list = driver.findElement(By.cssSelector("mentio-menu[mentio-search='bridgeSearch(term)']")).findElement(By.cssSelector("ul[class='dropdown-menu scrollable-menu']")).findElements(By.tagName("li"));
            int size = list.size();
    ...
    public static boolean checkVersion(WebDriver driver, WebDriverWait wait, List<Records> listOb, String className, ClientVersionDao clientVersionDao, int san_aut_id) throws Exception {
        ...
        try {
            wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("HelpglobalDivID")));
            driver.findElement(By.id("HelpglobalDivID")).click();
            Thread.sleep(3000);
            ...
            if(className.contains("4520") && version.equals("Version: 4.5.2.0"))
                ...
                actualOutcome=true;
            else if(className.contains("IpdsSanity3_2") && version.equals("Version: 3.2.0.0"))
                actualOutcome=true;
