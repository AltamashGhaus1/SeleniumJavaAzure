...
public class RowAndColumnLevel {
    ...
    public static void rowAndColumnLevel(WebDriver driver, WebDriverWait wait) throws Exception {
        ...
        driver.findElement(By.id("selectedIndex_0")).sendKeys(System.getenv("DATASET"));
        ...
        for(int i=0;i<dsSize;i++)
        {
            ...
            for(int j=0;j<webEl1.size();j++) {
                ...
                if((webEl.get(j).getText()).equalsIgnoreCase(System.getenv("DATASET")))
                {
                    webEl.get(j).click();
