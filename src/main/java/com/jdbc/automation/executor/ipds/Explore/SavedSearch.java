package com.jdbc.automation.executor.ipds.Explore;
...
public class SavedSearch {
    ...
    public static boolean savedSearch(WebDriver driver, WebDriverWait wait, String jdName, List<Records> listOb, ClientVersionDao clientVersionDao, int san_aut_id) throws Exception {
        ...
        try {
            ...
            wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("ExploreglobalImgID")));
            driver.findElement(By.id("ExploreglobalImgID")).click();
            ...
            try {
                List<WebElement> dsNameList = driver.findElement(By.cssSelector("div[class='sidebar-list']")).findElements(By.cssSelector("div[ng-show='indexPatternList.length > 1']"));
                List<WebElement> dsNameSubList = dsNameList.get(1).findElements(By.cssSelector("div[class='index-pattern']"));
                dsName = dsNameSubList.get(1).findElement(By.tagName("div")).getText().trim();

                System.out.println("DsName inside If: "+dsName);
            } catch (Exception e) {
            ...
            if (dsName.equals(jdName)) {
                ...
                try {
                    SeleniumUtils.changeDate(driver, wait,screenShot,"SavedSearch");
                } catch (Exception e) {
